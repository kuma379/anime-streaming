...
function getServerPref(): ServerPref {
  try {
    const raw = localStorage.getItem("anistream_server_pref");
    if (raw) return { defaultQuality: "720p", preferredServer: "vidhide", autoPlay: true, ...JSON.parse(raw) };
  } catch { /* ignore */ }
...
function VideoPlayer({ stream }: { stream: ActiveStream | null }) {
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden bg-black shadow-2xl shadow-purple-900/30">
      <iframe key={stream.url} src={stream.url} className="w-full aspect-video" allowFullScreen
        allow="autoplay; fullscreen; picture-in-picture; encrypted-media; clipboard-write"
        title="Stream Player" scrolling="no"
...
function groupByQuality(servers: EpisodeServer[]): GroupedServers[] {
  ...
  for (const s of servers) { const q = s.quality || "Lainnya"; if (!map[q]) map[q] = []; map[q].push(s); }
  return ["1080p","720p","480p","360p","Lainnya"].filter((q) => map[q]).map((q) => ({ quality: q, servers: map[q] }));
}

function findBestServer(groups: GroupedServers[], pref: ServerPref): EpisodeServer | null {
  ...
  const byName = (list: EpisodeServer[]) => list.find((s) => s.name.toLowerCase().includes(pref.preferredServer.toLowerCase()));
  return byName(embedInPrefGroup) || embedInPrefGroup[0] || byName(flatEmbed) || flatEmbed[0] || byName(prefGroup.servers) || prefGroup.servers[0] || null;
}
...
export default function Watch() {
  ...
  useEffect(() => {
    if (!slug) return;
    setLoading(true); setActiveStream(null); setServerError(null); setActiveServerId("");
    fetchEpisode(slug)
      .then((data) => {
        setEpisode(data);
      ...
      .catch(() => setServerError("Gagal memuat episode."))
      .finally(() => setLoading(false));
  }, [slug]);

  const autoLoadServer = async (server: EpisodeServer) => {
    setActiveServerId(server.serverId); setServerLoading(true);
    try { const { url } = await fetchServer(server.serverId); if (url) setActiveStream({ url, key: server.serverId }); }
  ...
  const handleServerClick = async (server: EpisodeServer) => {
    ...
    setActiveServerId(server.serverId); setServerLoading(true); setServerError(null);
    try {
      const { url } = await fetchServer(server.serverId);
      if (url) setActiveStream({ url, key: server.serverId });
      else setServerError("URL tidak tersedia. Coba server lain.");
    } catch { setServerError("Gagal memuat server. Coba server lain."); }
    finally { setServerLoading(false); }
  };

  if (loading) return (
    <div className="min-h-screen bg-[hsl(222,47%,5%)] flex items-center justify-center">
      <div className="text-center"><Loader2 className="w-10 h-10 text-purple-400 animate-spin mx-auto mb-3" /><p className="text-gray-400">Memuat episode...</p></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[hsl(222,47%,5%)]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          {episode && (<><span className="text-gray-600">/</span><span className="text-gray-300 text-sm line-clamp-1">{episode.title}</span></>)}
        </div>
        <AdBanner position="top" />
        <div>
          {serverLoading ? (
            <div className="w-full aspect-video bg-[hsl(222,47%,8%)] rounded-xl flex items-center justify-center">
              <div className="text-center"><Loader2 className="w-10 h-10 text-purple-400 animate-spin mx-auto mb-3" /><p className="text-gray-400 text-sm">Memuat stream...</p></div>
            </div>
            ...
            <div className="w-full aspect-video bg-[hsl(222,47%,8%)] rounded-xl flex items-center justify-center">
              <div className="text-center px-4">
                ...
                <button onClick={() => window.location.reload()} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-full text-sm mx-auto transition-colors">
        ...
        {episode && (
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-4">
              ...
              {episode.releaseTime && <span className="text-xs text-gray-500 shrink-0 mt-1">{episode.releaseTime}</span>}
            </div>
            <div className="flex flex-wrap items-center gap-2">
              {episode.prevEpisode?.episodeId && (
                <Link href={`/tonton/${episode.prevEpisode.episodeId}`} className="flex items-center gap-1.5 text-sm text-purple-400 hover:text-purple-300 transition-colors px-3 py-2 rounded-lg bg-[hsl(222,47%,12%)] hover:bg-purple-900/20">
                  <ChevronLeft className="w-4 h-4" />Sebelumnya
                </Link>
              )}
              {episode.nextEpisode?.episodeId && (
                <Link href={`/tonton/${episode.nextEpisode.episodeId}`} className="flex items-center gap-1.5 text-sm text-purple-400 hover:text-purple-300 transition-colors px-3 py-2 rounded-lg bg-[hsl(222,47%,12%)] hover:bg-purple-900/20">
                  Selanjutnya<ChevronRight className="w-4 h-4" />
                </Link>
              )}
              {episode.animeId && (
                <Link href={`/anime/${episode.animeId}`} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-white transition-colors px-3 py-2 rounded-lg bg-[hsl(222,47%,12%)] hover:bg-white/5">
                  Semua Episode
                </Link>
              )}
            </div>
            <div className="bg-[hsl(222,47%,8%)] rounded-2xl p-5 border border-purple-900/20">
              <div className="flex items-center gap-2 mb-4">
                ...
                {serverError && <span className="text-xs text-red-400 flex items-center gap-1 ml-auto"><AlertCircle className="w-3 h-3" />{serverError}</span>}
              </div>
              {grouped.length > 0 ? (
                <div className="space-y-4">
                  {grouped.map((group) => (
                    <div key={group.quality}>
                      <div className="flex items-center gap-2 mb-2">
                        <span className={`text-xs px-2 py-0.5 rounded border font-semibold ${QUALITY_COLORS[group.quality] || "bg-gray-800 text-gray-400 border-gray-700"}`}>{group.quality}</span>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {group.servers.map((server, i) => {
                          ...
                          return (
                            <button key={server.serverId} onClick={() => handleServerClick(server)} disabled={serverLoading}
                              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-all border ${isActive ? "bg-purple-600 text-white border-purple-500 shadow-lg shadow-purple-600/30" : "bg-[hsl(222,47%,12%)] text-gray-400 border-purple-900/20 hover:bg-purple-900/30 hover:text-white hover:border-purple-600/40"} disabled:opacity-60 disabled:cursor-not-allowed`}>
                              <Icon className="w-3.5 h-3.5" />
                              {server.name}
                              {canEmbed && <span className="text-[10px] text-green-400 font-normal">embed</span>}
              ...
              ) : (<p className="text-sm text-gray-500">Tidak ada server tersedia.</p>)}
            </div>
          </div>
        )}
